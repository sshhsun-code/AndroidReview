...
public class TextFlowLayout extends ViewGroup {
    ...
    private static final int MSX_LINE_NUMBER = 6;
    //用来存放所有的Line对象
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            if (lineList.size() == MSX_LINE_NUMBER){
                break;
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < lineList.size(); i++) {
            ...
            if (i < lineList.size() - 1) {
                mLastPerSpacing = perSpacing;//记录倒数第二行的平均空白值
            }
            ...
            for (int j = 0; j < viewList.size(); j++) {
                ...
                if (i < lineList.size() - 1) {
                } else {//如果是最后一行，那么平均空白值采用更上一行平均空白值比较小的值
                    widthSpec = MeasureSpec.makeMeasureSpec((int) (childView.getMeasuredWidth() + Math.min(perSpacing, mLastPerSpacing)), MeasureSpec.EXACTLY);
                }
    ...
    public int getVisibleChildCnt() {
        int visibleCnt = 0;
        for (Line line : lineList) {
            if (line == null || line.getViewList() == null) {
                continue;
            }
            visibleCnt += line.getViewList().size();
        }
        return visibleCnt;
    }
